  ...
  return (
    <section id="reviews" className="py-24 bg-white">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <Swiper modules={[Pagination, Autoplay]} pagination={{ clickable: true }} autoplay={{ delay: 5000, disableOnInteraction: false }} loop spaceBetween={32} breakpoints={{ 640: { slidesPerView: 2 }, 1024: { slidesPerView: 2 } }} className="pb-12">
          {reviews.map(review => (
            <SwiperSlide key={review.name}>
              <div className="bg-stone-50 p-8 h-full flex flex-col">
                <div className="flex mb-4">{[...Array(5)].map((_, i) => <span key={i} className="text-stone-400 text-sm">★</span>)}</div>
                ...
                <div className="flex items-center gap-4 pt-6 border-t border-stone-200">
                  <div className="w-10 h-10 bg-stone-900 flex items-center justify-center text-white text-sm font-light">{review.avatar}</div>
                  <div>
